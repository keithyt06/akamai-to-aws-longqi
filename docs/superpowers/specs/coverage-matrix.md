...
| A7 | **`tieredDistribution = true`**（Tier-2 回源聚合）| essl §6、api §6 | **Origin Shield** | ✅ (customer T1 2026-04-22) | phase0 T05（C2 加） |
...
| B4 | **`prefreshCache=90%` (stale-while-revalidate)** | essl §6、api §6 | 源站发 `Cache-Control: ..., stale-while-revalidate=Y`；CloudFront 支持 | ✅ | part2 T14（C3） |
...
| D9 | **`SEO tuning` / `PMUSER_TRIGGERED_RULES`** —— WAF 触发规则号回传源站 | essl §15 | AWS WAF labels → rule_label + custom_request_handling insert header `X-WAF-Rules-Triggered` | 🟡 (T9 待 Keith 解释后确认) | part4 T19（C6） |
...
| F2 | **`breakConnection: enabled=true`（api）** 故障注入 | api §10 | 需要客户确认是否保留；若保留，CloudFront 用 Lambda@Edge 或 WAF 特殊 rule 模拟 | 🟡 (T10 待 Keith 解释) | part4 T20（I5） |
...
## 统计（2026-04-22 客户回复后更新）

| 状态 | 原（初版 review） | 客户回复后 |
|---|---|---|
| ✅ Done | 38 | **50** |
| 🟡 Gap-declared（含待 Keith 解释的客户评估项）| 10 | **11** |
| 🔴 Todo | 13 | **0** |
| ➖ N/A | 5 | 5 |
| **合计** | **66** | **66** |

## 客户 2026-04-22 回复后新确认项（12 条 ✅）

- A4 (Host 透传)、A7 (Origin Shield)、B3 (api 扩展名分桶)、B4 (SWR)、B5 (stale-if-error)、B7 (Vary 差异)、B14 (/static/ token)、D5 (Timing-Allow-Origin)、D7 (modifyOutgoingRequestHeader = `Source-Auth: akamai-lqhair`)、D8 (Js tag = cacheTag 归并 ch12)、E4 (6 MRG 全启用)、E5 (Bot 按 path 两档 Common+Targeted)

## 仍待 Keith 解释后客户评估（🟡）

| # | 项 | 状态 |
|---|---|---|
| T2 | Origin Shield 区域选择 | 待 Keith 解释 Origin Shield 用途 |
| T3 | SWR / SIE 秒数 | 待 Keith 解释 SWR/SIE 机制 |
| D9 / T9 | PMUSER_TRIGGERED_RULES → X-WAF-Rules-Triggered | 待 Keith 解释 Akamai 用法 + AWS 能力 |
| F2 / T10 | breakConnection 保留与否 | 待 Keith 解释用途 |
| T12 | HSTS preload 不可逆 | 待 Keith 解释影响 |
| G2 | SureRoute PERFORMANCE | 待 Keith 解释作用 |
| G3 | Adaptive Acceleration | 待 Keith 解释作用 |
| G5 | TLS Fingerprint 规则 | 待 Keith 解释（客户暂接受） |
| G7 | Slow POST Protection | 待 Keith 解释作用 |
