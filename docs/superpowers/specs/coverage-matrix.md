...
| A4 | **`forwardHostHeader = REQUEST_HOST_HEADER`**（透传客户 Host）| essl §3、api §3 | CloudFront 不允许直接透传 Host；用 Function 注入 `x-viewer-host`，ALB listener rule 按此 header 分流 | 🔴→phase0 T05 修订 | phase0 T05（C1 fix） |
...
| A7 | **`tieredDistribution = true`**（Tier-2 回源聚合）| essl §6、api §6 | **Origin Shield** | 🔴→phase0 T05 修订 | phase0 T05（C2 加） |
...
| A10 | **`SureRoute PERFORMANCE`**（Akamai 动态路由）| essl §13、api §7 | 无精确等价；Origin Shield + Tiered Cache 部分替代 | 🟡→delivery §1.3 明示为非等价缺口 | delivery/01 |
| A11 | **`Adaptive Acceleration`**（MPulse 驱动 Push/Preconnect/Preload）| essl §8、api §8 | AWS 无原生等价 | 🟡→delivery Known Gaps | wrapup T26 |
...
| B3 | api 扩展名分桶（CSS/JS 365d、字体 365d、图片 30d、Files 7d、Other 7d、HTML 1d）| api §6 | **需要在 api Distribution 加相同的 ordered_cache_behavior** | 🔴→part2 T14 加子任务 | part2 T14（I8） |
| B4 | **`prefreshCache=90%` (stale-while-revalidate)** | essl §6、api §6 | 源站发 `Cache-Control: ..., stale-while-revalidate=Y`；CloudFront 支持 | 🔴→part2 T14 加子任务 | part2 T14（C3） |
| B5 | **`cacheError` ttl=10s preserveStale=true** | essl §6、api §6 | 源站发 `stale-if-error=Y` + CloudFront Custom Error Responses | 🔴→part2 T14 加子任务 | part2 T14（I1） |
| B6 | `validateEntityTag=false` | essl §6、api §6 | CloudFront 默认尊重 ETag；对齐 | ✅ | N/A |
| B7 | **`removeVary` 两侧差异（essl=true, api=false）** | essl §6 vs api §6 | www/m 侧用 Response Headers Policy 剔除 Vary；api 侧保留 | 🔴→part3 T17 加子任务 | part3 T17（I7） |
...
| B14 | **`/static/*` 特殊 query token `LT1RVf0XvMD1A78LUGJ2JvcSkHTKq8vb` (optional=true) 分版本** | essl §7 | Function 检测该 token，注入 `__static_ver` 入 cache key | 🔴→part2 T15 加子任务 | part2 T15（N1） |
...
| D2 | HSTS 2 年 + preload（v63/v11 staging）| essl §0 note、api §0 note | Response Headers Policy security_headers_config | ✅ | part3 T17 |
...
| D5 | **`Timing-Allow-Origin: *`** 响应头（给 mPulse RUM）| essl §17、api §14 | Response Headers Policy custom_headers_config | 🔴→part3 T17 加子任务 | part3 T17（C7） |
| D6 | 图片响应头 `Cache-Control` 365d | essl §19 | ch04 TTL 矩阵已覆盖 | ✅ | part2 T14 |
| D7 | **`modifyOutgoingRequestHeader` 指定回源请求头**（具体值在 raw JSON）| essl §17、api §14 | 需要人工读 raw JSON 后用 CloudFront Origin custom headers 或 Function 注入 | 🔴→part3 T17 加子任务（TODO） | part3 T17（I6） |
| D8 | **`Js tag` 注入**（向所有页面注入 JS）| essl §11 | CloudFront Function viewer-response 注入；先声明待定，客户确认 JS 内容后补 | 🔴→part3 T17 加子任务 | part3 T17（C5） |
| D9 | **`SEO tuning` / `PMUSER_TRIGGERED_RULES`** —— WAF 触发规则号回传源站 | essl §15 | AWS WAF labels → Function 读 label → 加请求头 `X-WAF-Rules-Triggered` | 🔴→part4 T19 加子任务 | part4 T19（C6） |
...
| E4 | **Application Layer Controls (OWASP / Core Rule Set)** | waf §4 `effectiveSecurityControls.Application=✅` | **AWS Managed Rule Group `AWSManagedRulesCommonRuleSet` + `AWSManagedRulesSQLiRuleSet`**（+ 按需 `KnownBadInputs`、`LinuxRuleSet`）| 🔴→part4 T18 新增子任务 | part4 T18（C4） |
| E5 | Bot Manager | waf §4 | `AWSManagedRulesBotControlRuleSet` (Common) —— 非 1:1 等价 | 🟡→delivery §10.3 | part4 T20 |
...
| E8 | Slow POST Protection | waf §4 | AWS 无原生；`size_constraint_statement` + CloudFront 超时 partial | 🟡→delivery §10.3 partial | part4 T20 |
...
| E11 | **TLS Fingerprint 系列（Client TLS Fingerprint）** | ops §6.3 | AWS WAF 不原生支持 JA3/JA4 | 🟡→delivery §9.3 gap | part4 T19 |
...
| F2 | **`breakConnection: enabled=true`（api）** 故障注入 | api §10 | 需要客户确认是否保留；若保留，CloudFront 用 Lambda@Edge 或 WAF 特殊 rule 模拟 | 🔴→part4 T20 加子任务 | part4 T20（I5） |
...
1. **phase0 T05 修订**：A4（Host 透传 bug）、A7（Origin Shield）
2. **part2 T14 加**：B3（api 扩展名分桶）、B4（prefreshCache）、B5（cacheError）
3. **part2 T15 加**：B14（/static/ token）
4. **part3 T17 加**：B7（removeVary 差异）、D5（Timing-Allow-Origin）、D7（modifyOutgoingRequestHeader）、D8（Js tag）
5. **part4 T18 加**：E4（OWASP Managed Rule Group）
6. **part4 T19 加**：D9（PMUSER_TRIGGERED_RULES / X-WAF-Rules-Triggered）
7. **part4 T20 加**：F2（breakConnection 确认）
